...
`interactionTokens` and writes `dist/theme.ts`. `dist/theme.ts` is generated and is never
edited by hand. A value that is not an engine name is a bug in the map, not a thing to
