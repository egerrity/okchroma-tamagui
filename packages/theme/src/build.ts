...
import { BASE, FAMILY_EDGE, TIERS, INPUT, DIALOG, POPOVER, INDICATOR_LEVELS, LEVEL_NAMES, type KeyMap } from './map.ts'
...
export function sourcesFor(colorNames: string[]): Record<string, KeyMap> {
  ...
  for (const mode of MODES) {
    ...
    for (const [name, map] of Object.entries({ ...INPUT, ...DIALOG, ...POPOVER })) sources[`${mode}_${name}`] = map
  }
  return sources
}
